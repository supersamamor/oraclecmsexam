...
{   
    ...
    {
		var result = await Mediatr.Send(DataRequest!.ToQuery<GetStocksQuery>());
    ...
    {
        var result = await Mediatr.Send(request.ToQuery<GetStocksQuery>(nameof(StocksState.Id)));
        return new JsonResult(result.ToSelect2Response(e => new Select2Result { Id = e.Id, Text = e.Id }));
    }
	public async Task<IActionResult> OnPostBatchUploadAsync()
    {        
        return await BatchUploadAsync<IndexModel, StocksState>(BatchUpload.BatchUploadForm, nameof(StocksState), "Index");
    }

    public IActionResult OnPostDownloadTemplate()
    {
        ModelState.Clear();
		BatchUpload.BatchUploadFileName = ExcelService.ExportTemplate<StocksState>(_uploadPath + "\\" + WebConstants.ExcelTemplateSubFolder);
